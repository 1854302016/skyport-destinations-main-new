...
const Detail = () => {
    ...
    // Force autoplay to start after data load
    useEffect(() => {
        if (data) {
            const timer = setTimeout(() => {
                if (gallerySwiperRef.current?.autoplay) {
                    gallerySwiperRef.current.autoplay.start();
                }
                if (similarSwiperRef.current?.autoplay) {
                    similarSwiperRef.current.autoplay.start();
                }
            }, 500);
    ...
    const galleryImages = data?.packagedetail?.package_gallery || [];
    const imageCount = galleryImages.length;

    const displayImages =
        imageCount > 0 && imageCount < 6
            ? [...galleryImages, ...galleryImages]
            : galleryImages;

    const getSlidesPerView = (base) => {
        if (imageCount === 0) return 1;
        if (imageCount <= 3) return Math.min(imageCount, base > 3 ? 3 : base);
        return Math.min(imageCount, base);
    };
    ...
    return (
        <motion.div 
            ...
            className="bg-[#ffffff] main_RESUSEA"
        >
            <div className="detail-page-header">
                <div style={{    background: "#efbf0414",
    padding: "20px 0px"}}>
                  <Container>
                    <motion.div variants={fadeInUp} className="header-breadcrumbs">
                        <span className="crumb">Home</span>
                        <span className="crumb-sep">/</span>
                        <span className="crumb">Tours</span>
                        <span className="crumb-sep">/</span>
                        <span className="crumb active">{data.packagedetail.name}</span>
                    </motion.div>
                    ...
                    <div className="header-top-info">
                        <motion.div 
                            ...
                            className="badge-row"
                        >
                            <span className="package-badge">BEST SELLER</span>
                            <span className="package-id">#{data.packagedetail.id}</span>
                        </motion.div>
                        ...
                        <motion.h1 
                            ...
                            className="main-package-title"
                        >
                            {data.packagedetail.name}
                        </motion.h1>
                        ...
                        <motion.div 
                            ...
                            className="header-meta-stylish"
                        >
                            <div className="meta-pill">
                                <WiDayCloudy /> 
                                <span>{data.packagedetail.days} Days / {data.packagedetail.nights} Nights</span>
                            </div>
                            <div className="meta-pill">
                                <GiNightSleep /> 
                                <span>{data.packagedetail.package_itinerary?.length || 0} Destinations</span>
                            </div>
                            <div className="meta-pill price-display d-md-none">
                                <span>From ${data.packagedetail.offer_price}</span>
                            </div>
                        </motion.div>
                    </div>
                </Container>
                ...
                <motion.div variants={fadeInUp} className="detail-gallery-container px-0 px-md-3">
                    {displayImages.length > 0 && (
                        <Swiper
                            onSwiper={(swiper) => (gallerySwiperRef.current = swiper)}
                            modules={[Autoplay, Navigation, Pagination]}
                            spaceBetween={15}
                            slidesPerView={getSlidesPerView(2.2)}
                            centeredSlides={true}
                            ...
                            breakpoints={{
                                576: { slidesPerView: getSlidesPerView(3.2) },
                                768: { slidesPerView: getSlidesPerView(4.2) },
                                1024: { slidesPerView: getSlidesPerView(5.2) },
                                1400: { slidesPerView: getSlidesPerView(6.2) },
                            }}
                                ...
                                <SwiperSlide key={index}>
                                    <div className="gallery-slide">
                                        <img
                                            ...
                                            alt={item.image_alt || data.packagedetail.name}
                ...
                <motion.div variants={fadeInUp} className="col-12 mx-auto d-block mt-5 pt-4">
                        ...
                        <svg className="svg-similar-reso mx-auto d-block" viewBox="0 0 370 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <defs>
                                ...
                                <linearGradient id="paint1_linear_2_2" x1="-2.0562e-07" y1="8.99999" x2={157} y2="8.99998" gradientUnits="userSpaceOnUse">
                                    ...
                                    <stop offset={1} stopColor="#efbf04" />
                                </linearGradient>
                            </defs>
                        </svg>
                    </div>
                    <div className="position-relative">
                        <Swiper
                            ...
                            loop={data.similar_packages?.length > 1}
                            className="similar-packages-swiper pb-5"
                        >
                        ...
                        <div className="custom-swiper-nav">
                            <button className="nav-btn prev" onClick={() => similarSwiperRef.current?.slidePrev()}>
                                <FaChevronLeft />
                            </button>
                            <button className="nav-btn next" onClick={() => similarSwiperRef.current?.slideNext()}>
                                <FaChevronRight />
                            </button>
                        </div>
                    </div>
